...
class Grad01(IOperator):
    """
    Simple implicit gradient operator excuted on `Grid` mesh in fvm.

    scheme:
        - implicit method.
        - central difference(CD) scheme for face interpolation
...
class Grad02(IOperator):
    ...
    scheme:
        - implicit method.
        - upwind scheme for face interpolation
    ...
    def prepare(self, mesh: Grid, **kwargs):
        if not isinstance(mesh, Grid):
            raise ValueError("Fvm Grad02 operator only supports Grid.")
    ...
    def run(self, source: Field) -> Field | LinearEqs:
        ...
        if src_type != VariableType.SCALAR and src_type != VariableType.VECTOR:
            raise ValueError(
                "Fvm Grad02 operator only supports scalar and vector fields."
            )
        if source.etype != ElementType.CELL:
            raise ValueError("Fvm Grad02 operator only supports cell fields.")
