...
class LinkableComponentStatus(Enum):
    ...
    After links between the component's inputs/outputs and those of other 
    components have been established, this component is validating 
    whether its required input will be available 
    when it updates itself, whether indeed it will be able to provide 
    the required output during this update.
    """
    ...
    input data from other components.
    """
    ...
    UPDATING = 9
    """
    The component is updating itself. It receives all required input
    data from other components, and is now performing the actual 
    computation. This state will end in status change to 
    `UPDATED`, `DONE` or `FAILED`.
    """
    ...
    FAILED = 14
    """
    The component was requested to perform the actions to be 
    performed before it will either be disposed or re-initialized again.
    The linkable component has failed initialize itself, 
    failed to prepare itself for computation, or failed to complete its
    update process. Typical actions would be writing the final 
