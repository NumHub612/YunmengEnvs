...
from yunmeng.solutions.HydrologicalSims.algorithms.Bases import RiverRouting
...
class MuskingumRouting(RiverRouting):
    ...
    def _coefficients(self, dt: float) -> tuple[float, float, float]:
        if self._coefs is not None and self._coefs[3] == dt:
            return self._coefs[:3]
        ...
        self._coefs = (c0, c1, c2, dt)
        return c0, c1, c2

    def state(self) -> dict:
        return {"I1": self._i1, "O1": self._o1}
